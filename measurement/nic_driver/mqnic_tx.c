...
netdev_tx_t mqnic_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	// myk review log
	netdev_info(ndev, "[mqnic_start_xmit]: Transmitting packet of length %d on device %s\n", skb->len, ndev->name);
	...
	if (unlikely(priv->if_features & MQNIC_IF_FEATURE_PTP_TS && shinfo->tx_flags & SKBTX_HW_TSTAMP)) {
		...
		printk(KERN_INFO "[mqnic][%s] Interface %d: TX TS requested\n", __func__, interface_index); // myk review log
