#!/usr/bin/env python3
# scripts/ssh_tofino_l2_bringup.py
...
def main():
    ...
    remote_script = f"{remote_dir}/tofino_apply_ports_and_l2_from_yaml_bfshell.py"

    local_remote_script = Path("scripts") / "tofino_apply_ports_and_l2_from_yaml_bfshell.py"
    if not local_remote_script.exists():
        raise FileNotFoundError(f"Missing local script: {local_remote_script}")

    logger.info("Preparing remote dir {}@{}:{}", user, host, remote_dir)
    sh(["ssh", f"{user}@{host}", "mkdir", "-p", remote_dir], check=True)

    logger.info("Copying topo + combined script to switch")
    ...
    # Use bash -lc to get a stable environment and allow sourcing SDE env.
    pre = ""
    if sde_env:
    ...
    logger.info("Executing remotely via bfshell -b (ports+L2)")
    cp = sh(["ssh", f"{user}@{host}", remote_cmd], check=False)

    (out_dir / "tofino_stdout.log").write_text(cp.stdout or "")
    (out_dir / "tofino_stderr.log").write_text(cp.stderr or "")

    if cp.returncode != 0:
        logger.error("Remote bfshell failed rc={}", cp.returncode)
        logger.error("See logs: {}/tofino_stdout.log and tofino_stderr.log", out_dir)
        raise SystemExit(cp.returncode)

    logger.info("Tofino ports+L2 bring-up completed. Logs: {}", out_dir)
